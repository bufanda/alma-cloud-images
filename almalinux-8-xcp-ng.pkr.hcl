...
source "qemu" "almalinux-8-xcp-ng-x86_64" {
  ...
  vm_name            = "AlmaLinux-8-XCP-NG-${var.os_ver_8}-${formatdate("YYYYMMDD", timestamp())}.x86_64.qcow2"
  cpu_model          = "host"
  cpus               = var.cpus
  efi_boot           = true
...
source "qemu" "almalinux-8-xcp-ng-aarch64" {
  ...
  net_device         = "virtio-net"
  qemu_binary        = var.qemu_binary
  vm_name            = "AlmaLinux-8-XCP-NG-${var.os_ver_8}-${formatdate("YYYYMMDD", timestamp())}.aarch64.qcow2"
  cpu_model          = "host"
  cpus               = var.cpus
  qemuargs = [
    ["-boot", "strict=on"],
...
source "qemu" "almalinux-8-xcp-ng-ppc64le" {
  ...
  net_device         = "virtio-net"
  qemu_binary        = var.qemu_binary
  vm_name            = "AlmaLinux-8-XCP-NG-${var.os_ver_8}-${formatdate("YYYYMMDD", timestamp())}.ppc64le.qcow2"
  cpus               = var.cpus
}
...
build {
  ...
  provisioner "ansible" {
    ...
    playbook_file        = "./ansible/xcp-ng.yml"
    ansible_env_vars = [
  ...
  provisioner "ansible" {
    ...
    only = [
      "qemu.almalinux-8-xcp-ng-aarch64",
      "qemu.almalinux-8-xcp-ng-ppc64le",
    ]
  }
}
